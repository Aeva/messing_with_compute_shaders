...
void main()
{
	...
	const float DistXY = distance(WorldPosition.xy, SphereParams.xy) / abs(SphereParams.w);
	if (DistXY > 1.0)
	{
		discard;
	}
	else
	{
		...
		const float Offset = sqrt(1 - DistXY * DistXY) * abs(SphereParams.w);
		const float NewDepth = Offset * Dir + SphereParams.z;
		const vec3 NewPosition = vec3(WorldPosition.xy, NewDepth);
		const vec3 Normal = normalize(NewPosition - SphereParams.xyz) * (SphereParams.w < 0 ? vec3(-1) : vec3(1));
